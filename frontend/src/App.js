...
// Define the main App component for the Hand Tracker Snake game
const App = () => {
  ...
  const gameObjectRef = useRef(null); // Ref for the SnakeGame instance
  ...
  useEffect(() => {
    ...
    const startGame = () => {
      if (!gameStartedRef.current) {
        startCamera()
          .then(() => {
            // Initialize a new SnakeGame instance
    ...
    document.getElementById('restart-btn').addEventListener('click', () => {
      ...
      gameObjectRef.current = new SnakeGame(canvas, gameStats); // Create a new game instance
      gameOver.style.display = 'none'; // Hide game over screen
